...
- (void)setShareSceneData:(HomeSceneListRow *)sceneModel {
    ...
    if (sceneModel.title.length == 0) {
        ...
        [self.title mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.equalTo(_sceneImg.mas_top).with.offset(-(SCREEN_WIDTH *0.2 + 28));
        }];
...
- (void)setUserTagBtn {
    ...
    for (NSInteger idx = 0; idx < self.tagDataMarr.count; ++ idx) {
        ...
        if (loc == 1) {
            userTag.frame = CGRectMake((btnX*(SCREEN_WIDTH*0.821)) - ((width+25)-18), btnY*(SCREEN_WIDTH*0.821)-32, width+25, 32);
        } else if (loc == 2) {
            userTag.frame = CGRectMake(btnX*(SCREEN_WIDTH*0.821)-44, btnY*(SCREEN_WIDTH*0.821)-32, width+25, 32);
        }
...
- (void)setViewUI {
    ...
    [_sceneImg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.mas_centerY).with.offset(16);
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH*0.821, SCREEN_WIDTH*0.821));
        make.centerX.equalTo(self);
    }];
    ...
    [_suTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(self);
        make.bottom.equalTo(_sceneImg.mas_top).with.offset(-SCREEN_WIDTH *0.2);
    }];
...
//    }];
    ...
    [_describe mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(_sceneImg.mas_top).with.offset(-SCREEN_WIDTH *0.05);
    ...
    [_qrImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.mas_bottom).with.offset(-SCREEN_WIDTH *0.10);
    }];
}
...
- (UIButton *)time {
    if (!_time) {
        ...
        _time.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
    }
    return _time;
}
