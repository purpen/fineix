...
- (void)viewDidLoad {
    ...
    if (SCREEN_HEIGHT == 812) {
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 88, SCREEN_WIDTH, SCREEN_HEIGHT-88) style:UITableViewStylePlain];
    } else {
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if (SCREEN_HEIGHT == 667.0) {
            ...
            return (422)/2+rect.size.height+40*SCREEN_HEIGHT/667.0;
        } else {
            return (422)/2+rect.size.height;
        }
    } else {
        return (144+40+SCREEN_WIDTH) * (self.modelAry.count - 1);
    }
    return 0;
}
