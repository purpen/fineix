...
//        UIButton * wayBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 70+ 60*idx, View_Width, 50)];
...
- (UIButton *)okPayBtn {
    if (!_okPayBtn) {
        ...
        _okPayBtn.backgroundColor = [UIColor colorWithHexString:@"#FF3366" alpha:1];
...
- (UIButton *)searchBtn {
    if (!_searchBtn) {
        ...
        [_searchBtn addTarget:self action:@selector(bindSearchAction) forControlEvents:(UIControlEventTouchUpInside)];
    }
    return _searchBtn;
}

//  点击搜索按钮
- (void)bindSearchAction {
...
//  购物车按钮
- (UIButton *)goodsCarBtn {
    if (!_goodsCarBtn) {
        _goodsCarBtn = [[UIButton alloc] init];
        [_goodsCarBtn setImage:[UIImage imageNamed:@"Cart"] forState:(UIControlStateNormal)];
        _goodsCarBtn.titleLabel.font = [UIFont systemFontOfSize:14];
        [_goodsCarBtn addTarget:self action:@selector(bindCartAction:) forControlEvents:(UIControlEventTouchUpInside)];
        
        [_goodsCarBtn addSubview:self.carNumLab];
        [_carNumLab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(15, 15));
            make.top.equalTo(_goodsCarBtn.mas_top).with.offset(5);
            make.right.equalTo(_goodsCarBtn.mas_right).with.offset(-5);
        }];
...
- (UILabel *)carNumLab {
    if (!_carNumLab) {
        ...
        _carNumLab.layer.cornerRadius = 15/2;
        _carNumLab.backgroundColor = [UIColor blackColor];
        _carNumLab.textAlignment = NSTextAlignmentCenter;
        _carNumLab.textColor = [UIColor whiteColor];
        _carNumLab.font = [UIFont systemFontOfSize:9];
    }
    return _carNumLab;
}

- (void)bindCartAction:(UIButton*)sender {
//    FBCartViewController *cartVC = [[FBCartViewController alloc] init];
