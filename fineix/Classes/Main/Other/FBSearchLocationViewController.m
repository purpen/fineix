...
- (void)viewDidLoad {
    ...
    self.tableV.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        ...
        NSString *urlString = [NSString stringWithFormat:@"http://api.map.baidu.com/place/v2/search?ak=20j0YXqCPWGqsPMTIcbF9Pv8&output=json&query=%@&page_size=10&page_num=%d&scope=1&region=%@",_address,m,@"全国"];
        m = m + 1;
...
(BMKReverseGeoCodeResult *)result
    ...
    if (searcher == self.codeSearch) {
        if (error == BMK_SEARCH_NO_ERROR) {
            ...
            NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
            AFURLSessionManager *manager = [[AFURLSessionManager alloc] initWithSessionConfiguration:configuration];
            NSString *urlString = [NSString stringWithFormat:@"http://api.map.baidu.com/place/v2/search?ak=20j0YXqCPWGqsPMTIcbF9Pv8&output=json&query=%@&page_size=10&page_num=0&scope=1&region=%@",result.address,@"全国"];
...
- (void)onGetGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKGeoCodeResult *)result errorCode:(BMKSearchErrorCode)error{
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
        AFURLSessionManager *manager = [[AFURLSessionManager alloc] initWithSessionConfiguration:configuration];
        NSString *urlString = [NSString stringWithFormat:@"http://api.map.baidu.com/place/v2/suggestion?query=%@&region=131&output=json&ak=20j0YXqCPWGqsPMTIcbF9Pv8",_searchTF.text];
